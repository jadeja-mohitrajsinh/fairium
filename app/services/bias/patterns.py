...
def _prepare_feature_matrix(dataframe: pd.DataFrame, target_column: str) -> pd.DataFrame:
    ...
    for column in features.columns:
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            # If median is NaN (all-NaN column), fall back to 0
            fill_value = median if not pd.isna(median) else 0.0
            encoded[column] = numeric.fillna(fill_value)
        else:
            encoded[column] = LabelEncoder().fit_transform(series.astype(str).fillna("missing"))

    # Final safety net: fill any remaining NaNs with 0
