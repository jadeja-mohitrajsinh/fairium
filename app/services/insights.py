...
async def build_enhanced_explanation(
    sensitive_column: str,
    group_rates: Dict[str, float],
    dp_diff: float,
    di_ratio: float,
    ...
    """Build human-readable explanation with context, optionally using AI."""
    if not group_rates:
        return f"No valid groups found for sensitive column '{sensitive_column}'."
    
    # Try to use Gemini AI for enhanced insights if available
    if use_ai and os.getenv("GOOGLE_API_KEY"):
        try:
            ...
            metric_data = {
                ...
                "di_ratio": di_ratio,
                "dp_diff": dp_diff,
                "confidence": "HIGH" if all(g["total_samples"] >= 30 for g in group_analysis) else "LOW",
                "severity": classify_severity(dp_diff),
            }
            ai_insight = await gemini_service.generate_bias_insight(metric_data)
            return ai_insight
        except Exception as e:
            print(f"AI insight generation failed, falling back to rule-based: {e}")
